 ...
 *   @(#) $Header: /home/plasmacvs/source_tree/ivutils/include/vector_set.h,v 1.3 2012/06/29 10:50:12 valuev Exp $
 *
 *****************************************************************************/
/*
$Source: /home/plasmacvs/source_tree/ivutils/include/vector_set.h,v $
$Revision: 1.3 $
$Author: valuev $
$Date: 2012/06/29 10:50:12 $
*/
/*s****************************************************************************
 * $Log: vector_set.h,v $
 * Revision 1.3  2012/06/29 10:50:12  valuev
...
/*
 Before there are definition of some classes that can be used
 ...
 SurfaceVectorSet is class which store all elements of the sentense.
...
// This class stores all elements of the sentense.
struct SurfaceVectorSet{

  vector<Vector_3> pos; // vector sequence
  vector<Vector_3> ds; // surface elements sequence

  class iterator{ // iterator on current element of the sequence
    friend struct SurfaceVectorSet;
    const SurfaceVectorSet *parent;
    int  i; // current number

    // if e=1 returns the end iterator, otherwise returns iterator on the first element
    iterator(const SurfaceVectorSet *sparent,int e=0): parent(sparent){
...
/// sequence corresponding to the grid at the sphere surface.
...
template<class vset_it_t>
class transformed_vset_it: public base_vset_it{
  vset_it_t it;
...
template<class vset_it_t>
base_vset_it *make_virt_vset_it(vset_it_t it, mngarg<VecTransform> transform =NULL, mngarg<VecTransform> transform_ds=NULL ){
  if(transform.ptr() && transform_ds.ptr())
    return new transformed_vset_it<vset_it_t>(it,transform,transform_ds);
  else
    return new virt_vset_it<vset_it_t>(it);
}
