...
AtomVecWavepacket::AtomVecWavepacket(LAMMPS *lmp) : AtomVec(lmp)
{
  ...
  //atom->ecp_flag = 0; // compatible with eff. disable ECP TODO: Check ecp flag
...
void AtomVecWavepacket::grow(int n)
{
  ...
  cs = memory->grow(atom->cs,nmax, 2,"atom:cs");
  csforce = memory->grow(atom->csforce,nmax, 2,"atom:csforce");
  vforce = memory->grow(atom->vforce, nmax, 3, "atom:vforce");
...
void AtomVecWavepacket::create_atom(int itype, double *coord)
{
  ...
  etag[nlocal] = 0; // to be assigned later     //atom->nlocal; //electron should be different
  cs[nlocal][0] = 1.; //Packet should exist by default
  cs[nlocal][1] = 0.;
                                  ...
                                  char **values)
{
  ...
  cs[nlocal][0] = atoi(values[6]);
  cs[nlocal][1] = atof(values[7]);
...
int AtomVecWavepacket::data_atom_hybrid(int nlocal, char **values)
{
  ...
  cs[nlocal][0] = atoi(values[4]);
  cs[nlocal][1] = atof(values[5]);
...
double AtomVecWavepacket::memory_usage()
{
  double bytes = 0;
