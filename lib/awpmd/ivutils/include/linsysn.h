...
/// @file linsysn.h \brief Interface for solving systems of linear equations using LAPACK and PARDISO
...
#elif defined(USE_LAPACK) // LAPACK is linked from some lib file (no MKL)
...
*/
template<class T>
int linsys_cpp(T *matr,T *vect,int num,T *x,int lda=0);
...
int print(matr_t &matr){
  ...
  for(int i=0;i<matr.sz();i++){
    for(int j=0;j<matr.sz();j++){
//      fprintf(f,"%g%c",matr.get_m(i,j),9);
...
public:
  ...
  void subvector(vect_solver &other,int i0){
    vect=other.vect+i0;
    SZ=n;
  }
...
// or by Gauss method using function linsys_cpp (otherwise).
template<class T>
