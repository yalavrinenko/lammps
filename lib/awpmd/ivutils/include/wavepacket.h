...
// Constants
const double MIN_EXP_ARG = -80; //-35.;  // Minimum noticeable argument for exp function
...
public:
  ...
  ///\en Returns the integral of w(x) over 3D space
  cdouble integral() const {
    cdouble z = lz + b.norm2()/(4.*a)+ (3./2)*log(M_PI/a);
	...
	///\en Returns integral of w(x) over 3D space along selected axes.
  ///    Flags \a integ_flag are used to indicate along which axes the integration should be performed.
	///    For example the call with integ_flag = iVector_3(0,1,1)  will return integral over y and z at x[0].
  cdouble partial_integrate(const Vector_3 &x, const iVector_3 &integ_flag) const {
		cdouble psum(0.);
		cdouble lg = 0.5*log(M_PI/a);
		for(int i=0;i<3;i++){
			if(integ_flag[i]) // integration
        psum+= b[i]*b[i]/(4*a) + lg;
			else // value
			  psum+= -a*x[i]*x[i]+ b[i]*x[i];
  ...
  ///\en Initialize normalized packet with physical parameters: r0, p0, width, pw
  ...
  /// Initialize normalized packet with complex parameters a and b
  /// w(x)=(3/2pi width^(3/4)exp[-(3/(4 width^2)-i pw/(2*width) )(x-r)^2+i p (x-r)]
  ...
  cdouble operator()(const Vector_3 &x) const {
    return exp(lz - a*x.norm2() + b*((cVector_3)x));
  }

  /// Adjusts lz so that Integral[w*(conj(w))] is 1 after this operation
