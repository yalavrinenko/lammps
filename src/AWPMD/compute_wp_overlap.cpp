...
// Based on compute_rdf.cpp (Copyright (2003) Sandia Corporation)
//

#include "compute_wp_overlap.h"
...
ComputeWPOverlap::ComputeWPOverlap(LAMMPS *lmp, int narg, char **arg) :
  Compute(lmp, narg, arg),
  wppair(nullptr), nwppair(nullptr), ilo(nullptr), ihi(nullptr), jlo(nullptr), jhi(nullptr),
  hist(nullptr), histall(nullptr), typecount(nullptr), icount(nullptr), jcount(nullptr),
  duplicates(nullptr)
{
  if (narg < 4) error->all(FLERR,"Illegal compute wp_overlap command");

  array_flag = 1;
  extarray = 0;

  nbin = utils::inumeric(FLERR,arg[3],false,lmp);
  if (nbin < 1) error->all(FLERR,"Illegal compute wp_overlap command");
  ...
  if (nargpair == 0) npairs = 1;
    ...
    if (nargpair % 2) error->all(FLERR,"Illegal compute wp_overlap command");
    npairs = nargpair/2;
  }
  ...
  memory->create(wppair,npairs,ntypes+1,ntypes+1,"wp_overlap:wppair");
  memory->create(nwppair,ntypes+1,ntypes+1,"wp_overlap:nwppair");
  ilo = new int[npairs];
  ihi = new int[npairs];
  jlo = new int[npairs];
  jhi = new int[npairs];
  ...
  if (nargpair == 0) {
    ...
    for (int ipair = 0; ipair < npairs; ipair++) {
      ...
      if (ilo[ipair] > ihi[ipair] || jlo[ipair] > jhi[ipair])
        error->all(FLERR,"Illegal compute wp_overlap command");
      iarg += 2;
    }
  }
  ...
  memory->create(hist,npairs,nbin,"wp_overlap:hist");
  memory->create(histall,npairs,nbin,"wp_overlap:histall");
  memory->create(array,nbin,1+2*npairs,"wp_overlap:array");
...
ComputeWPOverlap::~ComputeWPOverlap()
{
  memory->destroy(wppair);
  memory->destroy(nwppair);
  delete [] ilo;
  delete [] ihi;
  delete [] jlo;
  delete [] jhi;
...
void ComputeWPOverlap::init()
{

  if (!force->pair && !cutflag)
    error->all(FLERR,"Compute wp_overlap requires a pair style be defined "
               "or cutoff specified");
...
void ComputeWPOverlap::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
void ComputeWPOverlap::init_norm()
{
  int i,j,m;
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      for (ihisto = 0; ihisto < ipair; ihisto++) {
        m = wppair[ihisto][itype][jtype];
        hist[m][ibin] += 1.0;
      }
      if (newton_pair || j < nlocal) {
        for (ihisto = 0; ihisto < jpair; ihisto++) {
          m = wppair[ihisto][jtype][itype];
          hist[m][ibin] += 1.0;
        }
      }
    }
