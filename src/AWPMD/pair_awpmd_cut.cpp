...
LAMMPS_NS::PairAWPMD::awpmd_packets LAMMPS_NS::PairAWPMD::make_packets() const
{
  ...
  if (wp_per_electron > 1) {
    ...
    if (tagv.size()) {
      ...
      int group_start = -1, last_tag =0, last_spin=0;
      for (
        size_t i = 0; i < tagv.size();
        i++) {    // assuming wavepackets were created using groups with size of multiple of wp_per_electron
        int myid = tagv[i].second;
        if (group_start > 0 && (tagv[i].first != last_tag + 1 || spin[myid]!=last_spin)) // group is a number of electrons with consecutive tags
          group_start = -1;  // new group started
        if (group_start < 0)
          group_start = tagv[i].first;
        last_tag = tagv[i].first;
        last_spin = spin[myid];
        newetag[myid] = 1+ (last_tag - group_start) / wp_per_electron;
      }
