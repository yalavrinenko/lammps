...
Packet Molecular Dynamics (WPMD) method with density functional theory extension :ref:`[1] <wpmddft>`.

WPMD-DFT uses Hartree approximation (same as in :doc:`pair_style wpmd/cut <pair_wpmd>`) for evaluation of Coulomb interaction. The additional exchange-correlation
...
The *gppn* keyword sets the number of gpu per one node. This is required for correct task scattering to multiple gpus.
