...
# endif
  ...
  bool skip_by_tag_order(int s1, int ic1, int c1, int s2, int ic2, int c2) const {
    ...
    if (tag1 <= 0 && tag2 <= 0) { // all at other partition
      if (tag1 == tag2) // always accept same tags
        res = false;
      else
        res = true;
    }
    ...
    else if (tag2 <= 0)  // c2 is ghost, always skipped
      res = true;
    else // c1 is ghost, always kept
      res = false;
...
public:
  ...
  virtual double interaction_ee_single(WavePacket const &packet_1, WavePacket const &packet_2, double **eforce,
                                       double **erforce);

  virtual double interation_ii_single(int i, int j, double **x, double *q, double** f);

  virtual double
  interaction_ei_single(double *x, double q, WavePacket const &packet, int spin, double *f, double *eforce,
                        double *erforce);
...
//# if DFT_EXTENSION
		if (mesh_nx != dft_meshx || mesh_nz != dft_meshz || mesh_nz != dft_meshz || max_packets !=dft_conf.packet_number) { // same parameters?
          ...
          if (custom_approx != nullptr)
			...
			dft_conf.space_size = {1.8f*(float)cell[0], 1.8f*(float)cell[1], 1.8f*(float)cell[2]};
			dft_conf.mesh_step.size.as_struct = {float(dft_conf.space_size.x / dft_conf.mesh_size.size.as_struct.x),
                                           float(dft_conf.space_size.y / dft_conf.mesh_size.size.as_struct.y),
                                           float(dft_conf.space_size.z / dft_conf.mesh_size.size.as_struct.z)};
			dft_conf.packet_number = max_packets;
			dft_conf.space_shift = {(float)(center[0]- dft_conf.space_size.x/2),
                              (float)(center[1]- dft_conf.space_size.y/2),
                              (float)(center[2]- dft_conf.space_size.z/2)}; // assuming equal sizes for all directions!!!
