...
public:
  ...
  double interaction_ee_single(WavePacket const &packet_1, WavePacket const &packet_2, double **eforce,
                               double **erforce) override;

  double interaction_ei_single(double *x, double q, WavePacket const &packet, int spin, double *f, double *eforce,
                               double *erforce) override;
...
protected:
  ...
  ///    inculded in the prefactor (not included automatically).  When
  ///    \a external=1, the contribution is attributed to the external
  ///    force and F_extra is updated (not added to internal
  ///    force). Otherwise E_der is updated.
