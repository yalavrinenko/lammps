...
// a small number, large enough to calculate 1/tiny
const double tiny = 1e-20, tiny2 = tiny*tiny;
// large enough so that 1.0+eps > 1.0, when using the floating point arithmetic
double eps = 1e-15, eps2 = eps*eps;
// constants
const double sqrt_pi_1 = 1. / sqrt(M_PI), sqrt_pi_2 = 2. * sqrt_pi_1;
  ...
  do
    {
      ...
      if (D.real() == 0.0 && D.imag() ==  0.0) D = tiny ;
...
const Complex cerf_series( const Complex z )
{
  ...
  for (int n=0; n<3 || norm(term) > norm(sum)*eps2; n++)
    {
      sum = sum + term / (double)(2*n+1) ;
      term = -term * z2 / (double)(n+1) ;
    }

  return sum * sqrt_pi_2;
}
  ...
  for (int np=-35; np<=35; np+=2)
    {
      ...
      Complex b( exp(t*t) / (double)(np+n0) ) ;
      sum += b ; 
    }

  sum = sum * exp(-z*z) * 2. / M_PI ;
