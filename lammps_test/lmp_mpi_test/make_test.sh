...
test_case (){
	...
	mpirun -n 4 $1 -in lmp.in | grep "Step TotEng" -A1 | tail -n 1
	mpirun -n 8 $1 -in lmp.in | grep "Step TotEng" -A1 | tail -n 1
}
...
test_case_eff (){
	...
	mpirun -n 4 $1 -in hartree.in | grep "Step TotEng" -A1 | tail -n 1
	mpirun -n 8 $1 -in hartree.in | grep "Step TotEng" -A1 | tail -n 1
}
